...
(function ($) {
    ...
    createTicketForm.addEventListener('submit', function (event) {
        ...
        fetch(createTicketForm.action, {
            method: 'POST',
            body: formData,
            // headers are not needed for FormData with anti-forgery token
        })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    bootstrap.Modal.getInstance(supportModal).hide();
                    app.showSuccessAlert('ส่งเรื่องสำเร็จ!', {
                    ...
                    app.showErrorAlert('ส่งเรื่องไม่สำเร็จ', { html: errorMessages });
                }
            })
            .catch(error => {
                console.error('Form submission error:', error);
                app.showErrorAlert('เกิดข้อผิดพลาดในการเชื่อมต่อ', { text: 'กรุณาลองใหม่อีกครั้ง' });
            })
            .finally(() => {
                // Hide loading state
